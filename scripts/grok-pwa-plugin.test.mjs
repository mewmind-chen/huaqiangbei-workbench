...
/**
 * Tests for chrome fallbacks must not inherit this application's own branding
 * files. Production supplies its root explicitly; an empty temporary root
 * keeps these unit tests focused on the fallback under test.
 */
function isolatedPwaContext(context = {}) {
  return {
    cwd: mkdtempSync(join(tmpdir(), "grok-pwa-unit-")),
    ...context,
  };
}
...
test("platform chrome overwrites share-card metas and always sets og:title", () => {
  ...
  const out = injectGrokPwaHead(html, isolatedPwaContext({ appName: "Wild Race" }));
...
test("published grok.me slug is still a title fallback", () => {
  const out = injectGrokPwaHead("<html><head></head></html>", isolatedPwaContext({
    host: "wild-race.grok.me",
  }));
...
test("injects into documents with no head element", () => {
  const out = injectGrokPwaHead(
    ...
    isolatedPwaContext({ appName: "Solo" }),
  );
...
test("streaming injector matches </HEAD> case-insensitively", () => {
  const injector = createHeadInjector(isolatedPwaContext({ appName: "Wild Race" }));
  const chunks = [
...
test("uses the app name in the injected title tag", () => {
  const out = injectGrokPwaHead(
    "<html><head></head></html>",
    isolatedPwaContext({ appName: "Wild Race" }),
  );
